...
def test_prv_002b_ind_create_prompt_dataset(driver, sample_csv_path, base_url, test_credentials):
    ...
    assert isinstance(prov_home, ProviderHomePage), (
        "test_prv_002b: go_to_login(flow='provider') did not return ProviderHomePage"
    )

    # Step 2: Go to My Dashboard
    my_dash = prov_home.goto_my_dashboard()
    assert isinstance(my_dash, MyDashboardPage), (
        "test_prv_002b: goto_my_dashboard() did not return MyDashboardPage"
    )
    ...
    assert isinstance(create_ds, CreateDatasetPage), (
        "test_prv_002b: click_add_new_prompt_dataset() did not return CreateDatasetPage"
    )
    ...
    test_description = f"Automated prompt dataset test {timestamp}"
